 ...
 * background tabs. Nothing is guessed: a file without a known location links nowhere.
 */
...
import { closeTab, newTab, renderTabBar } from './tabs.js';
...
async function open(root) {
  ...
  const visited = new Set(session.tabs.map(tabKey).filter(Boolean));
  const queue = [root];
  const opened = [];
