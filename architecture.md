...
| `schema.SchematronValidator`, `schema.LocatedDocument`, `schema.Severity` | The Schematron evaluator over the JDK's XPath 1.0 engine (`javax.xml.xpath`): the `ns` prefixes, the phase (the one asked, else `defaultPhase`, else every pattern), abstract patterns instantiated by textual substitution of their `$param`s in every expression, abstract rules whose assertions and `let`s are collected by `extends` (recursively, each once), `let`s at the schema / pattern / rule levels evaluated into a variable resolver (a node-set stays a `NodeList`), `include`s replaced by the root of the file next to the schema (files only, depth ≤ 16). A rule's context (an XSLT pattern) becomes a selection from the root: `//` before each top-level alternative not starting at `/`; a node fires the first rule of a pattern that selects it. Messages are rendered with `value-of` evaluated and `name` the context node's name, the diagnostics the assertion names appended. An expression that does not compile is reported once as `unsupported`. The document is parsed through `LocatedDocument` (SAX → DOM, the start-tag position kept as user data on each element, namespace declarations kept as attributes), so a problem carries the line and column of its context node and its path (`/po:purchaseOrder/po:items/po:item[2]/po:quantity`). Each problem names the assertion, rule and pattern that fired with the node ids of `SchematronParser` (`elementIds()`), which is what lets the page select them. `Severity` is the vocabulary: `error`, `warning`, `info` (from the `role` / `flag`), `unsupported`. |
...
| `GET /api/capabilities` | – | `{"dialogs": bool, "language": "fr", "version": "3.6.0", "javaVersion": "21.0.12"}`: whether the server can show native file dialogs (not headless) — the page disables the workspace commands and falls back to the browser's file dialog otherwise — the language of the machine's locale (the page's default language), and the versions shown by Help ▸ About (`BuildInfo`: the jar manifest's `Implementation-Version`, "dev" without one). |
