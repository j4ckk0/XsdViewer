...
| Class | Role |
|---|---|
| `XsdViewerApplication` | Entry point: takes the host, port and verbosity from `ConfigFile` and the saved port, parses the `CommandLineOptions` over them, checks the initial file, opens the log file where `ConfigFile` says, starts the `XsdViewerServer` (with the automatic stop unless `--keep-alive` / `--no-browser`), prints the URL and opens the browser. |
| `CommandLineOptions` | Record of the options (`--port`, `--host`, `--no-browser`, `--keep-alive`, `--verbose`, `-h`, optional `file.xsd`) and their parser, which starts from given defaults so a command-line option overrides `xsdviewer.ini`; `stopWhenNoPage()` = neither `--keep-alive` nor `--no-browser`. |
| `ConfigFile` | The optional `xsdviewer.ini` (working directory, else beside the jar): the host and port defaults, the verbosity and the folder of the log files (`none` for the console only), so the double-clicked launcher can be configured without a command line. A `key=value` line per setting; a bad value is warned about and the default kept. |
| `BrowserLauncher` | Opens a URL with `java.awt.Desktop`, falling back to `xdg-open`. |
| `Log` | The log (`java.util.logging`): console and, once `openFile` is called with the folder `ConfigFile` gives (the temporary directory by default, null for none), a pair of rotating `xsdviewer.%g.log` there; `INFO` and up — what happens to the server and what fails — or, with `--verbose` (`Log.setVerbose`), `FINE` too: each request received and each schema parsed; every handler failure is logged with its stack trace by `XsdViewerServer` and answered as a 500 with a message. |
| `UserSettings` | The settings changed from the page's Settings menu, kept in the user's `java.util.prefs.Preferences` (`org/jtools/xsdviewer`): `autoStop`, and the `port` chosen for the next start (0 = the default). The command line wins for a run. |
...
| `schema.NodeKind`, `schema.LinkLabel`, `schema.XsdNames`, `WsdlNames`, `SchematronNames` | The constants of the model: the kinds of node, the edge labels, and the namespace / element / attribute names each parser reads. |
...
| `server.XsdViewerServer` | Starts a `com.sun.net.httpserver.HttpServer` bound to `127.0.0.1:8091` by default and maps each path of `ApiPath` to its handler. Requests are handled on virtual threads (`Executors.newVirtualThreadPerTaskExecutor`). |
...
| `server.SettingsHandler` | `GET` / `POST /api/settings`: `{"autoStop": bool, "port": n}`, each field optional in a `POST` — `autoStop` applied to `PageWatch.setEnabled` at once, `port` kept for the next start (a server keeps the port it already listens on) — both kept through `UserSettings`. |
...
| `js/constants.js`, `js/dom-names.js`, `js/dom.js`, `js/message-keys.js` | The strings of the client: the API contract and vocabulary shared with the server (kinds, paths, storage keys); the names of the page's elements, CSS classes and data attributes (`dom-names.js`, a catalogue) and the helpers that use them (`dom.js`: `$`, `esc`, `selector`, `dataAttr`, `legendHtml`); the keys of the texts. The vocabulary only — what is made of it (what a link is) belongs to `js/link-categories.js`. |
...
| `js/schema-index.js`, `js/declaration-lookup.js` | Indexing a parsed schema into a tab; looking declarations up across the files of a workspace, open in a tab or only listed (`findIn`, `findInWorkspace`, `usersInWorkspace`, `placeOfEntry`, `locationsFor`) — never across workspaces. |
...
| `js/model-view.js` | The Model view: `layout()` gives each leaf a row and centres a parent on its children, `modelSvg()` draws the elbow connectors and the boxes (`boxSvg()`) — the comparison of two declarations draws each side with it, and the exports take the SVG as they take the graph's. `modelTree()` asks the server for the tree once, whichever views wait for it: the Graph view tints the objects the model walks through (`openedIn()`) and asks for the model when it is not at hand. The module wires its own buttons and canvas (`initModelView(select, showView)`, called from `app.js` with what selects a declaration and what shows a view: a box's ◎ handle shows its object in the graph, as a node's ▤ handle shows its model). |
...
| `js/png-export.js`, `js/text-export.js`, `js/text-diff-picture.js`, `js/file-download.js` | The exports. `png-export.js` serves the views that are drawings: the SVG cropped to what it holds, with the page's CSS and background put under it so the file renders alone (`standalone()`), the comparison exported as one picture holding its two panes — the content models, the neighbourhoods, or the sources — under the heading each pane carries, and the PNG being that SVG rasterised. `text-export.js` paints a file's source line by line onto a canvas, that Text view being HTML rather than a drawing; `text-diff-picture.js` rebuilds the comparison's text in SVG — the line numbers, the lines, the backgrounds of the deleted, inserted and moved ones, the folds — in the page's colours and font, so the comparison exports the same way in its three views. `file-download.js` hands the file to the browser. |
...
- **No hard-coded strings in the logic.** Names that carry meaning live in constants classes /
  ...
  `LinkLabel`), the XSD vocabulary (`XsdNames`), element ids / CSS classes / data
...
│   └── src/main/java/org/jtools/xsdviewer/
│       ├── XsdViewerApplication, CommandLineOptions, ConfigFile, BrowserLauncher, Log, UserSettings, BuildInfo
