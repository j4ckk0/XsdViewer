...
| Class | Role |
|---|---|
| `XsdViewerApplication` | Entry point: takes the host and port from `ConfigFile` and the saved port, parses the `CommandLineOptions` over them, checks the initial file, starts the `XsdViewerServer` (with the automatic stop unless `--keep-alive` / `--no-browser`), prints the URL and opens the browser. |
| `CommandLineOptions` | Record of the options (`--port`, `--host`, `--no-browser`, `--keep-alive`, `-h`, optional `file.xsd`) and their parser, which starts from given defaults so a command-line option overrides `xsdviewer.ini`; `stopWhenNoPage()` = neither `--keep-alive` nor `--no-browser`. |
| `ConfigFile` | The optional `xsdviewer.ini` (working directory, else beside the jar): the host and port defaults, so the double-clicked launcher can be configured without a command line. A `key=value` line per setting. |
