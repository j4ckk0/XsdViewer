...
| `js/file-list.js`, `js/sidebar.js`, `js/graph.js`, `js/details.js`, `js/text-view.js`, `js/xml-highlighter.js`, `js/png-export.js` | One module per view: the Files panel (the active workspace's tabs as a tree by folder, redrawn with the tab bar), schema header (foldable) and object list, SVG ego-graph, details panel (collapsible to a strip), source text, its tokenizer, the PNG export. Folded states are remembered in `localStorage`. |
