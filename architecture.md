...
| `js/tabs.js`, `js/page.js` | Workspaces and their tabs (`newWorkspace`, `newTab(ws)`, `tabsOf(ws)`, activate / close a tab or a workspace, `closeAllTabs`, the tab bar with one group per workspace); `renderPage()` redraws everything from the active tab, `showView()` switches graph / text. |
