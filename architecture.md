...
| `js/link-filter.js` | The *Links* menu of the graph toolbar: the categories of link the graph draws (`LINK_CATEGORY`, `linkCategory()` in `constants.js`: content, attributes, types, base types, references, and a WSDL's or a Schematron's chain — the family's chain wins as soon as one end of the link belongs to it). A category off is kept in `localStorage`, the button is marked while one is, and the menu shows only the entries the shown file can have (the classes of the legend). `renderGraph()` drops the rows of a hidden category, at both levels and on both sides. |
| `js/panels.js` | The widths of the side panels: one splitter (`role="separator"`, focusable) between the sidebar and the main area, another between the main area and the details panel. A pointer drag — or the arrow keys — sets the panel's width, clamped between `PANEL.MIN_WIDTH` and a share of the window, and kept in `localStorage`; a double-click drops the stored width, back to the stylesheet's. The graph is laid out for the width it has, so it is redrawn when a drag ends. `updateSplitters()` (called by `renderPage()` and by the details panel's collapse) hides the splitter of a panel that is not shown. |
