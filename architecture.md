...
| `js/switch-menu.js`, `js/link-filter.js`, `js/kind-filter.js` | The two switch menus of the graph toolbar, built on one factory (`switchMenu()`: the values off kept in `localStorage`, the check marks, the button marked while something is off, the entries shown being those of the shown file's family, as the legend says). *Links*: the categories of link (`LINK_CATEGORY`, `linkCategory()` in `constants.js`: content, attributes, types, base types, references, and a WSDL's or a Schematron's chain — the family's chain wins as soon as one end of the link belongs to it). *Types*: the kinds of object, the built-in types among them. `renderGraph()` drops the rows of a hidden category or kind, at both levels and on both sides; the centre is always drawn. |
