...
- **The compare view exports three pictures**: the left declaration alone, the right alone, and the two side by side as before. PNG and SVG both.
...
developer can build on. Everything the page did in JavaScript — reading a schema, building the
content model of a declaration, comparing two of them — is Java in `org.jtools:xsdviewer-core`,
served by an API the page itself uses, with examples for both. Nothing changes for who reads
schemas: the jar runs alone as before and the page behaves exactly as it did.
...
- **The comparison and the content model are the server's, and an API.** The tree the Model view draws, the alignment of two declarations' trees, the line diff with its moved blocks and business-lines filter, the schema diff — everything the page computed in JavaScript is now Java in `core` (`model.ContentTree`, `compare.LineDiff`, `compare.BusinessLines`, `compare.SchemaDiff`, `compare.ModelDiff`), served by five stateless endpoints — `POST /api/model`, `/api/compare/declarations`, `/api/compare/texts`, `/api/compare/schemas`, `/api/compare/workspaces` — whose requests carry the files' texts, so one call is the whole story. The page asks and draws; nothing it did changes. A program using the API alone gets the same trees, marks and diffs (`CompareApiTest` shows the calls).
- **Examples for developers.** `examples/` is a module of the build: four programs on the library (reading a schema, the model of a declaration, comparing two, validating a document) and five JavaScript programs on the HTTP API, all run against `samples/`, tested with every build so they cannot go stale. The README opens on two doors: the application, for who reads schemas; the library and the API, for who writes programs.
- **The code review's findings.** The library exports what it is for (`model` and `compare` join `schema` and `json` in the module descriptor) and each package says in its own documentation what is promised and what is internal. The comparison rules left the HTTP handlers for the library — `TextComparison` (which lines are compared, on what they are matched) and `WorkspacePairing` (the files of two workspaces by name, each pair's status) — so the handlers only read a request and write an answer, and `CompareJsonWriter` and `BoxJsonWriter` shape those answers, the model and diff classes knowing nothing of JSON. On the page, a module's name now says what kind of thing it is: `comparison-state.js`, `files-section.js`, `objects-section.js`, `workspace-selection.js`, `declaration-lookup.js`, `view-router.js` beside `page.js`, and `dom-names.js` (the catalogue of the page's names) beside `dom.js` (what does something with them).
- **`XsdVocabulary`, `WsdlVocabulary` and `SchematronVocabulary` are `XsdNames`, `WsdlNames` and `SchematronNames`**: they hold the words each specification defines — what a file being read is written in — not texts a reader sees. The `schema` package now says which of its two families of constants is which: the languages read, and the model produced, which the page mirrors.
- **The documentation says what the tool now is.** `architecture.md` opens on four diagrams — the two artifacts and who depends on which, what happens when a schema is read, the library's packages, the places the page shows — and its design choices no longer describe a tool that reported imports without following them. The README carries the same picture and a sixth screenshot: two declarations compared as text.
- **Fix: the Model view could stay marked as loading**, which anything watching that mark waited on for ever, when a tab or a selection changed while the server was answering; and a file opened again with another content was answered from the comparison made before.
