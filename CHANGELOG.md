...
- **Examples for developers.** `examples/` is a module of the build: four programs on the library (reading a schema, the model of a declaration, comparing two, validating a document) and five scripts on the HTTP API, all run against `samples/`, tested with every build so they cannot go stale. The README opens on two doors: the application, for who reads schemas; the library and the API, for who writes programs.
