...
Checked against the tree at version 2.0.0 (August 2026).
...
| **Tags and releases** | Users download a version, not a commit. | `git tag -a v2.0.0 -m "2.0.0"` and push the tag; create a GitHub Release from it, attach `xsdviewer.jar` and the two `-Pdist` archives (they are ~50 MB each: fine for Release assets, never for the repository). |
...
| **`CHANGELOG.md`** | The git log already records the versions (1.3.0 … 2.0.0); a short file per version is friendlier. | Optional; GitHub Release notes can serve instead. |
...
5. Locally:
   ...
   git tag -a v2.0.0 -m "2.0.0" && git push github v2.0.0
   ```
   The existing `origin` (the bare repository on the NAS) keeps working; `git push origin` and `git push github` are independent.
6. On GitHub: set the description and topics; *Releases ▸ Draft a new release* from `v2.0.0`, attach `target/xsdviewer.jar`, `target/xsdviewer-2.0.0-windows.zip` and `target/xsdviewer-2.0.0-linux.tar.gz` (built with `./package.sh`), paste the release notes.
