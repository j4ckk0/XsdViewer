...
|---|---|---|---|
| 1 | **A licence file** (`LICENSE` at the root) | Without one, the code is "all rights reserved": people may read it but not legally use, copy or modify it. GitHub detects the file and shows the licence on the repository page. Common choices: MIT (short, permissive), Apache-2.0 (permissive + patent grant), GPL-3.0 (copyleft). | **Missing.** Choose one; also declare it in `pom.xml` (`<licenses>`) and in a "Licence" section of `README.md`. |
...
| 4 | **Attribution of third-party content** | Copying code or data from elsewhere requires respecting its licence. | `samples/purchaseOrder.xsd` is adapted from the W3C *XML Schema Part 0: Primer* purchase-order example (W3C Document Licence: reuse allowed with attribution). The file header says so; add the source URL and the W3C copyright line to it, or to a `NOTICE` file. `samples/import/*.xsd` are original. The only dependency, JUnit (EPL-2.0), is test-scoped and not redistributed. |
| 5 | **Licences of what the release archives bundle** | `mvn package -Pdist` ships an Eclipse Temurin JRE (GPLv2 with Classpath Exception). Redistribution is allowed provided its notices travel with it. | OK as long as the `jre/` directory is copied whole (it is: `legal/` and `release` are inside). Say in `README.md ▸ Packaging` that the archives bundle Temurin and under which licence; do not commit the archives themselves (they are git-ignored). |
...
| **`pom.xml` metadata** | Shown by Maven tooling and GitHub's dependency graph. | Add `<url>`, `<licenses>`, `<scm>` (the GitHub URL), `<developers>`. |
...
1. Decide the licence; add `LICENSE`, the `<licenses>` block in `pom.xml`, a "Licence" section in `README.md`. Complete the W3C attribution in `samples/purchaseOrder.xsd` (or `NOTICE`). Commit.
2. Decide on the author e-mail and the commit trailers (item 3). If they must change, rewrite the history now (`git filter-repo`), before anything is pushed.
3. (Optional) add `.github/workflows/build.yml`, `docs/screenshot.png`, the `pom.xml` metadata. Commit.
